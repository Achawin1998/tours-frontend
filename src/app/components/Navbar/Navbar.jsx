...
function Navbar() {
    ...
    const toggleMenu = () => { // นำ menuRef ที่ใช้ toggle มาใส่ฟังชัน toggle และเพิ่ม class เอาไปใส่ในตัว icon hamburger และ ตัว link ที่ใช้ map 
              ...
              <div className='container_flex gap-4'>
                {/* logo */}
                    <div className='logo-img'>
                        <Link href={'/home'}>
                             <Image src={logo} className='logo' alt='' width={''} height={''} />
                        </Link>
                    </div>
                {/* logo end */}
                ...
                {/* Menu start */}
                    <div className='navigation' ref={menuRef} onClick={toggleMenu}>
                        ...
                        return (
                        <li key={index} className={isActive ? 'active__link' : 'nav__item'}>
                            <Link href={link.href}>
                                {link.name}
                            </Link>
                        </li>
                ...
                <div className='d-flex align-items-center gap-4 btn__etc '>
                         {user ? (
                            <>
                                ...
                                <Button className='btn btn_logout' onClick={LogOut}>LogOut</Button>
                            </>
                        ): (
                            <>
                                <Button className='btn btn_register '><Link href={'/login'}>LogIn</Link></Button>
                                <Button className='btn btn_login'><Link href={'/register'}>Register</Link></Button>
                            </>
                        )}
                 </div>
                {/* end */}
